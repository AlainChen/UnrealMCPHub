...
它的原则是：
- 先把核心 3D 能力域都跑一遍
- 每个域先做一个轻量、可复用、可展示、且保持全自动的 baseline
- 先建立广度，再决定哪些域值得 advanced
- 在至少 3 个 baseline 跑通之前，不进入重型 advanced 路线

这里的 `baseline` 有明确边界：
- 目标是验证“最小可用自动化能力”
- 不追求一次覆盖复杂系统、重型 sample 或完整 cinematic pipeline
- 不把高风险自动化链路和能力验证绑在同一轮里
- 先选低风险、结构清晰、可重复的自动化任务
...
当前进度：
- `Gym-01`: baseline-ready
- `Gym-02`: baseline-ready
- `Gym-03`: baseline brief defined, next recommended item

当前已知底座状态：
- `RemoteMCP` 已完成一轮 `P0` / 初始 `P0.5`，覆盖
  - map lifecycle
  - scene/testbed construction
  - evidence capture
  - health / reconnect baseline
- 这意味着 Gym 现在的主要问题不再是“完全没有底层工具”，而是
  - 如何把这些能力回接到 Hub/workflow
  - 如何把 baseline gym 真正做成稳定、可读、可展示的实例
...
2. 完成 `Gym-02` 的第一份 before / after 证据包
3. 启动 `Gym-03`
4. 为 `Gym-04` 到 `Gym-05` 补齐 baseline brief
5. 等至少 3 个域有第一版 showcase 之后，再评估 `Advanced Track`
