"""Test session_tools (add_note, get_session)."""
...
class TestAddNote:
    ...
    @pytest.mark.asyncio
    async def test_add_note_no_instance(self, tmp_home):
        mcp = FastMCP("test")
        store = StateStore()
        register_session_tools(mcp, lambda: store)
        tools = {t.name: t.fn for t in mcp._tool_manager.list_tools()}
...
class TestGetSession:
    @pytest.mark.asyncio
    async def test_notes_scope_empty(self, tmp_home):
        store, tools = _setup(tmp_home)
    ...
    @pytest.mark.asyncio
    async def test_history_scope_empty(self, tmp_home):
        store, tools = _setup(tmp_home)
    ...
    @pytest.mark.asyncio
    async def test_full_scope_json(self, tmp_home):
        store, tools = _setup(tmp_home)
        import json
    ...
    @pytest.mark.asyncio
    async def test_only_two_tools_registered(self, tmp_home):
        store, tools = _setup(tmp_home)
        assert set(tools.keys()) == {"add_note", "get_session"}
